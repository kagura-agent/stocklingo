  ...
  return (
    <>
      <div className="px-6 pt-10 pb-24">
        <h1 className="text-2xl font-black mb-4">术语表</h1>
        ...
        <div className="sticky top-0 z-10 bg-white py-3">
          <input
            ...
            className="w-full rounded-xl border-2 border-duo-gray-200 px-4 py-3 text-sm outline-none focus:border-duo-green transition-colors"
          />
          <p className="mt-2 text-xs text-duo-gray-300">
        ...
        <div className="mt-4 space-y-6">
          {Object.entries(grouped).map(([group, entries]) => (
            <section key={group}>
              <h2 className="text-sm font-bold text-duo-gray-400 mb-2">{group}</h2>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {!isOpen && (
                            <p className="text-xs text-duo-gray-300 mt-1 line-clamp-1">
                              {entry.content}
                            </p>
                          )}
                        </div>
                        <span className="text-duo-gray-300 text-xs shrink-0">
                          {isOpen ? "▲" : "▼"}
                        </span>
                      </div>
                      {isOpen && (
                        <div className="mt-3 space-y-2 text-sm">
                          <p className="text-duo-gray-400">{entry.content}</p>
                          {entry.funFact && (
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-duo-gray-300">
              没有找到相关术语
            </div>
          )}
        </div>
